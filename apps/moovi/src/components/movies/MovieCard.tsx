    ...
    return (
      ...
      >
        <div className="aspect-[2/3] bg-cpc-grey-900 overflow-hidden relative">
          ...
          <div hidden={!!posterUrl} className="w-full h-full flex items-center justify-center text-cpc-green-900 text-[10px]">
            NO POSTER
          </div>
          {movie.personal_rating && (
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-black/70 px-1.5 py-1">
            ...
            <div className="text-cpc-green-900 text-[9px] truncate">{movie.year}{movie.director ? ` - ${movie.director}` : ''}</div>
          </div>
        </div>
      </div>
    )
