...
export function useRecommendations() {
  ...
  const selectHistoryEntry = useCallback(
    (index: number) => {
      ...
      setTokens(
        entry.tokens_used
          ? { input_tokens: 0, output_tokens: 0, total_tokens: entry.tokens_used }
          : null,
      );
      setEvents([]);
      setError(null);
  ...
  const startStream = useCallback(
    (
      streamFn: typeof streamRecommendations,
      movieIds: string[] = [],
      vibe: number = 50,
      yearFrom?: number,
      yearTo?: number,
  ...
  const generate = useCallback(
    (movieIds: string[] = [], vibe: number = 50, yearFrom?: number, yearTo?: number) => {
      startStream(streamRecommendations, movieIds, vibe, yearFrom, yearTo);
    },
    [startStream],
  );

  const generateSimple = useCallback(
    (movieIds: string[] = [], vibe: number = 50, yearFrom?: number, yearTo?: number) => {
