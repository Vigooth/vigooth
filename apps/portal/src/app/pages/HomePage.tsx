  ...
  return (
    <CpcLayout>
      <div className="h-full overflow-auto flex flex-col">
        ...
        <div className="text-center py-4 border-b-2 border-cpc-green-500">
          <div className="text-cpc-yellow-500 text-2xl font-bold">VIGOOTH SYSTEM</div>
          <div className="text-cpc-cyan-500 text-sm mt-1">
            Type CAT to list programs, RUN &quot;name&quot; to launch
          </div>
        </div>

        {/* Terminal */}
        <div
          className="flex-1 p-4 font-mono text-sm cursor-text text-left"
          onClick={() => inputRef.current?.focus()}
        >
          {history.map((line, i) => (
            <div key={i} className="text-cpc-green-500 leading-relaxed whitespace-pre">
              {line}
            </div>
          ))}
        ...
        {/* Quick access */}
        <div className="border-t-2 border-cpc-green-500">
          <div className="text-cpc-green-900 text-xs px-4 py-2">Quick access:</div>
          <AppMenu apps={appsConfig} />
        </div>

        <div className="text-center py-2 border-t-2 border-cpc-green-500 text-cpc-green-900 text-xs">
