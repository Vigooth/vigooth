...
function CollapsedSidebar({ friendCount, onExpand }: { friendCount: number | null; onExpand: () => void }) {
  return (
    <div
    ...
    >
      ...
      {friendCount !== null && (
        <div className="text-cpc-green-900 text-[10px]">{friendCount}</div>
      )}
    </div>
  )
}
...
export function FriendsSidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile drawer */}
      <div className={`lg:hidden fixed top-0 right-0 bottom-0 z-50 w-72 bg-cpc-grey-900 border-l-2 border-cpc-magenta-500 flex flex-col transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {sidebarContent}
      </div>
