 ...
 */
export function getEntriesForFolder(
  vault: VaultData | null,
  folderId: string | null,
): PasswordEntry[] {
  if (!vault) return [];
  return vault.entries.filter((e) => (folderId ? e.folderId === folderId : !e.folderId));
}
...
export function countEntriesInFolder(vault: VaultData | null, folderId: string | null): number {
  if (!vault) return 0;
  return vault.entries.filter((e) => (folderId ? e.folderId === folderId : !e.folderId)).length;
}
