...
const judgments = () => {
  const judgmentArr = ['coupable', 'innoncent'];
  return judgmentArr.map(judgment => <li key={judgment} className="Thumb-judgment-item"><a>{judgment}</a></li>)
};
    ...
    {fugitives.map(fugitive =>
      <div key={fugitive._id} className="Thumb">
        <div className="Thumb-img">
          <img className="Thumb-img-item" src={fugitive.src} alt="" />
        </div>
