...
function resolve(config: IconConfig, size: Size, variant: IconVariant): VariantDef {
  const exact = config.variants.find((v) => v.size === size && v.variant === variant);
  ...
  const def = config.variants.find(
    (v) => v.size === config.default.size && v.variant === config.default.variant,
  );
  if (!def) throw new Error(`[createIcon] Default variant not found in config for icon`);
  return def;
}
