...
export function Terminal({ onCommand, prompt = 'READY' }: TerminalProps) {
  ...
  return (
    <div
      className="bg-cpc-grey-900 text-cpc-green-500 font-cpc text-sm p-4 min-h-screen cursor-text overflow-hidden relative border border-cpc-magenta-900"
      onClick={handleClick}
    >
      <div className="whitespace-pre-wrap">
        ...
        <div className="flex items-baseline leading-tight relative">
          <span className="text-cpc-green-500 mr-1">&gt;</span>
          <span className="font-cpc text-cpc-green-500">{currentInput}</span>
          <input
            ref={inputRef}
            type="text"
            value={currentInput}
